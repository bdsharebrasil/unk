...
type DJ = Tables<'profiles'>;
type DJInsert = TablesInsert<'profiles'>;
type DJUpdate = TablesUpdate<'profiles'>;
...
export class DJService {
  ...
  static async create(djData: Omit<DJInsert, 'id' | 'created_at' | 'updated_at'>): Promise<DJ> {
    const nameForLog = (djData as { artist_name?: string; full_name?: string }).artist_name || (djData as { full_name?: string }).full_name || 'DJ';
    console.log('✨ Criando novo DJ:', nameForLog);

    try {
      const dataWithRole = { ...djData, role: 'dj' };
      const { data, error } = await supabase
        .from('profiles')
        .insert(dataWithRole)
        .select()
  ...
  static async uploadProfileImage(djId: string, file: File): Promise<string | null> {
    ...
    try {
      ...
      await this.update(djId, { avatar_url: publicUrl } as DJUpdate);
  ...
  static async toggleActive(id: string): Promise<boolean> {
    ...
    try {
      ...
      console.log('✅ Status do DJ alternado com sucesso');
      toast.success('Status do DJ alternado com sucesso!');
      return true;
    } catch (error: any) {
