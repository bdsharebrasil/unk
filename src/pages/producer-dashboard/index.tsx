...
import { LogOut, FileText, Plus, Calendar } from "lucide-react";
...
const ProducerDashboard: FC = () => {
  ...
  const fetchContracts = useCallback(async (producerId: string) => {
    ...
    try {
      ...
      // Handle missing table error from PostgREST (PGRST205)
      if (String(msg).includes('contract_instances') || String(msg).includes('PGRST205')) {
        console.warn('contract_instances table missing or schema metadata not available:', msg);
        setContracts([]);
      } else {
        console.error('Error fetching contracts:', msg);
        setContracts([]);
      }
    } finally {
