...
const Select = ({ label, value, options, onChange, triggerClassName = "" }: any) => (
  <div className="grid gap-2">
    ...
    <select
      ...
      className={cn("flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", triggerClassName)}
    >
      {options.map((opt: any) => (
...
type Note = { id: string; user_id: string; title: string; content: string | null; folder_id: string | null; created_at: string; updated_at: string };

type NoteFolder = { id: string; user_id: string; name: string; color: string; created_at: string };
const folderColors = ['#a855f7', '#3b82f6', '#22c55e', '#f59e0b', '#ef4444', '#06b6d4', '#e11d48', '#84cc16'];
...
const NotesSection = ({
  ...
  notes: Note[];
  folders: NoteFolder[];
  onCreateNote: (values: { title: string; content: string; folder_id: string | null }) => void;
  onUpdateNote: (id: string, values: Partial<Pick<Note, 'title' | 'content' | 'folder_id'>>) => void;
  onDeleteNote: (id: string) => void;
  onCreateFolder: (values: { name: string; color: string }) => void;
  onUpdateFolder: (id: string, values: Partial<Pick<NoteFolder, 'name' | 'color'>>) => void;
  ...
  const visibleNotes = notes
    .filter(n => (activeFolder === 'all' ? true : (n.folder_id || '') === activeFolder))
  ...
  const startCreateNote = () => {
    ...
    setTitle('');
    setContent('');
    setNoteFolderId(activeFolder === 'all' ? '' : activeFolder);
  ...
  const submitNote = () => {
    ...
    if (editingNote) {
      onUpdateNote(editingNote.id, { title: title.trim(), content, folder_id: noteFolderId || null });
      toast({ title: 'Nota atualizada' });
    } else {
      onCreateNote({ title: title.trim(), content, folder_id: noteFolderId || null });
  ...
  const submitFolder = () => {
    if (!folderName.trim()) {
      toast({ title: 'Informe um nome para a pasta', variant: 'destructive' });
      return;
    }
    if (editingFolder) {
      ...
      onCreateFolder({ name: folderName.trim(), color: folderColor });
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
      <aside className="liquid-glass p-4 h-max rounded-xl">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Folder className="h-5 w-5 text-primary" />
            <h3 className="font-semibold">Pastas</h3>
          </div>
          <Button size="sm" onClick={startCreateFolder} className="btn-primary">
            Nova
          </Button>
        </div>
        <Input placeholder="Buscar notas..." value={search} onChange={(e: any) => setSearch(e.target.value)} className="mb-3" />
        <div className="space-y-1">
          <button
            className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-lg text-left smooth-transition', activeFolder === 'all' ? 'bg-white/10' : 'hover:bg-white/5')}
            onClick={() => setActiveFolder('all')}
          >
            <span className="inline-block w-2 h-2 rounded-full bg-foreground/50"></span>
            Todas as Notas
          </button>
          {folders.map(f => (
            <div key={f.id} className="flex items-center justify-between group">
              <button
                className={cn('flex-1 flex items-center gap-2 px-3 py-2 rounded-lg text-left smooth-transition', activeFolder === f.id ? 'bg-white/10' : 'hover:bg-white/5')}
                onClick={() => setActiveFolder(f.id)}
              >
                <span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: f.color }}></span>
                {f.name}
              </button>
              <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 pr-1">
                <button className="p-1 hover:text-primary" onClick={() => startEditFolder(f)} title="Editar"><Edit2 className="w-4 h-4" /></button>
                <button className="p-1 hover:text-destructive" onClick={() => onDeleteFolder(f.id)} title="Excluir"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">{activeFolder === 'all' ? 'Minhas Notas' : (folders.find(f => f.id === activeFolder)?.name || 'Minhas Notas')}</h2>
          <Button onClick={startCreateNote} className="gap-2 gradient-primary hover-lift smooth-transition border-0 text-white"><Plus className="h-4 w-4" /> Nova nota</Button>
        </div>
        <div className="grid gap-3">
          {visibleNotes.map(n => (
            <Card key={n.id} className="glass-card hover-lift smooth-transition">
              <CardContent className="pt-4 smooth-transition">
                  ...
                  <div>
                    ...
                    {n.folder_id && (
                      <div className="mt-2 text-xs text-muted-foreground flex items-center gap-2">
                        <span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: folders.find(f => f.id === n.folder_id)?.color || '#999' }}></span>
                        {folders.find(f => f.id === n.folder_id)?.name}
                      </div>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => startEditNote(n)}>Editar</Button>
                    <Button size="sm" variant="destructive" onClick={() => onDeleteNote(n.id)}>Excluir</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <DialogContent className="max-w-xl glass-card">
          ...
          <div className="grid gap-3">
            ...
            <Select
              label="Pasta"
              value={noteFolderId}
              onChange={(v: string) => setNoteFolderId(v)}
          ...
          <DialogFooter className="mt-4">
            ...
            <Button onClick={submitNote}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-md glass-card">
          ...
          <div className="grid gap-3">
            <div className="grid gap-2">
              ...
              <Input id="folder-name" value={folderName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFolderName(e.target.value)} />
            </div>
            <div className="grid gap-2">
              <Label>Cor</Label>
              <div className="flex flex-wrap gap-2">
                {folderColors.map(c => (
                  <button key={c} onClick={() => setFolderColor(c)} className={cn('w-7 h-7 rounded-full border', folderColor === c && 'ring-2 ring-primary')} style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>
          </div>
          <DialogFooter className="mt-4">
            ...
            <Button onClick={submitFolder}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
const PersonalAgendaSection = ({
  ...
  const eventsForDay = useMemo(() => {
    return items.filter((item) => isSameDay(safeParseDate(item.date), selectedDate)).sort((a, b) => {
  ...
  const getItemsFor = (d: Date) => items
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-1 rounded-lg glass-effect p-1">
          <Button size="sm" variant={viewMode === 'calendar' ? 'default' : 'ghost'} className={viewMode === 'calendar' ? 'btn-primary' : 'btn-ghost-glass'} onClick={() => setViewMode('calendar')}>Calendário</Button>
          <Button size="sm" variant={viewMode === 'list' ? 'default' : 'ghost'} className={viewMode === 'list' ? 'btn-primary' : 'btn-ghost-glass'} onClick={() => setViewMode('list')}>Lista</Button>
        </div>
            ...
            <CardContent className="overflow-x-auto">
              <div className="grid grid-cols-7 min-w-[700px] text-xs font-semibold text-muted-foreground">
                {['SEG','TER','QUA','QUI','SEX','SAB','DOM'].map((d) => (
                  <div key={d} className="p-2 text-center liquid-glass rounded-md">{d}</div>
                ))}
              </div>
                ...
                {buildMonthGrid.map((cell, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative h-24 sm:h-28 w-full p-2 text-left overflow-hidden liquid-glass rounded-md",
                        !cell.inCurrent && "opacity-70",
            ...
            <CardContent>
              <div className="space-y-4">
                {eventsForDay.map((item) => (
                  <div key={item.id} className="liquid-glass p-4 rounded-xl">
          ...
          <CardContent>
            <div className="grid gap-3">
              {items
                ...
                .map((item) => (
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 liquid-glass rounded-lg p-3">
                    <div>
                      <p className="text-sm font-semibold">{item.title}</p>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Select
                ...
                onChange={(value: string) => setFormData((prev) => ({ ...prev, status: value as AgendaItem["status"] }))}
              />
              <Select
                label="Prioridade"
                value={formData.priority}
                options={priorityOptions}
                onChange={(value: string) => setFormData((prev) => ({ ...prev, priority: value as AgendaItem["priority"] }))}
              />
            </div>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Select
                ...
                options={categoryOptions}
                onChange={(value: string) => setFormData((prev) => ({ ...prev, category: value as AgendaItem["category"] }))}
              />
              <div className="grid gap-2">
...
const DjDetailView = ({ 
  ...
  const djId = dj?.id;
  const { data: events = [], loading: eventsLoading } = useSupabaseData(eventService, 'getByDj', [djId], [djId]);
      ...
      <Tabs defaultValue="events" className="w-full">
        ...
        <TabsContent value="events" className="space-y-4">
          ...
          {!eventsLoading && events.map((event: any) => (
            <div key={event.id} className="p-3 bg-muted/30 rounded-lg border border-border hover:bg-muted/50 transition-colors">
              <p className="text-sm font-medium">
                {event.title || event.event_name || 'Evento sem título'} - {event.event_date ? new Date(event.event_date).toLocaleDateString('pt-BR') : 'Data não definida'}
              </p>
            </div>
          ))}
        </TabsContent>
...
const AgendaManager = () => {
  ...
  const [personalItems, setPersonalItems] = useLocalStorageState<AgendaItem[]>(STORAGE_KEYS.personal, getInitialPersonalItems());
  const [contentItems, setContentItems] = useLocalStorageState<AgendaItem[]>(STORAGE_KEYS.content, getInitialContentItems());
  ...
  const createNote = useCallback(async (values: { title: string; content: string; folder_id: string | null }) => {
    if (!user?.id) return;
    const { data, error } = await (supabase as any)
      .from('manager_notes')
  ...
  const updateNote = useCallback(async (id: string, values: Partial<Pick<Note, 'title' | 'content' | 'folder_id'>>) => {
  ...
  const createFolder = useCallback(async (values: { name: string; color: string }) => {
    if (!user?.id) return;
    const { data, error } = await (supabase as any)
      .from('manager_note_folders')
      .insert({ user_id: user.id, ...values })
      .select()
      .single();
    if (!error && data) setFolders(prev => [...prev, data as NoteFolder]);
  }, [user?.id]);

  const updateFolder = useCallback(async (id: string, values: Partial<Pick<NoteFolder, 'name' | 'color'>>) => {
    const { error } = await (supabase as any)
      .from('manager_note_folders')
      .update(values)
      .eq('id', id);
    if (!error) {
      setFolders(prev => prev.map(f => f.id === id ? { ...f, ...values } as NoteFolder : f));
    }
  }, []);

  const deleteFolder = useCallback(async (id: string) => {
    await (supabase as any).from('manager_note_folders').delete().eq('id', id);
    setFolders(prev => prev.filter(f => f.id !== id));
    setNotes(prev => prev.map(n => (n.folder_id === id ? { ...n, folder_id: null } : n)) as Note[]);
  }, []);
  ...
  const updatePersonalStatus = useCallback((id: string, status: AgendaItem["status"]) => {
  ...
  const updateContentItem = useCallback((id: string, data: Partial<AgendaItem>) => {
  ...
  const changeKanbanGroupBy = (groupBy: KanbanGroupBy) => {
    ...
    toast({ title: "Visualização Kanban atualizada", description: `Agrupado por: ${groupBy.charAt(0).toUpperCase() + groupBy.slice(1)}` });
  };
          ...
          <div className="grid lg:grid-cols-[1fr_300px] gap-6">
            <div className="space-y-6">
              <div className="flex items-center gap-4 p-4 rounded-xl border bg-muted/30">
                <Label htmlFor="kanban-group-by" className="whitespace-nowrap font-semibold">Agrupar Kanban por:</Label>
                <Select
