...
const EventCalendar = () => {
  ...
  const calendarEvents = useMemo<ExtendedCalendarEvent[]>(() => {
    ...
    const events = (Array.isArray(rawEvents) ? rawEvents : [])
      .map((event: any) => {
        ...
        // Buscar apenas DJs da tabela event_djs (sem DJ principal)
        const allDJs = Array.isArray(event?.event_djs)
        ...
        // determine primary DJ object (from event.dj or event.event_djs[0].dj)
        const primaryDj = event?.dj ?? (Array.isArray(event?.event_djs) && event.event_djs.length > 0 ? event.event_djs[0]?.dj : null) ?? null;
        const creatorName = (event as any)?.creator?.full_name || (event as any)?.creator?.artist_name || (event as any)?.creator?.email || null;
        ...
        return {
          ...
          dj: primaryDj ? { id: primaryDj.id ?? primaryDj?.dj_id ?? null, name: primaryDj.artist_name ?? primaryDj.name ?? null, genre: primaryDj.genre ?? null, email: primaryDj.email ?? null } : null,
          ...
          created_by_admin: Boolean((event as any)?.creator?.role === 'admin' || (event as any)?.creator?.is_admin === true),
          creator_name: creatorName,
        };
      })
      .filter(Boolean) as ExtendedCalendarEvent[];
    // Remover eventos temporários de debug
    return events;
  ...
  const resolveDj = useCallback(
    async (djId: string): Promise<CalendarDJ | null> => {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select("id, artist_name, email, full_name")
        .eq("id", djId)
        .eq("role", "dj")
        .maybeSingle();
      ...
      return {
        id: String(data.id),
        name: data.artist_name || data.full_name || data.email || "DJ",
        email: data.email ?? "",
      };
    },
    [calendarDjs],
  ...
  const resolveProducer = useCallback(
    async (producerId: string): Promise<CalendarProducer | null> => {
      ...
      return {
        id: String(data.id),
        name: data.full_name || data.email || "Produtor",
        email: data.email ?? "",
        company_name: null,
      };
    },
  ...
  const handleDeleteEvent = useCallback(
    async (eventId: string) => {
      ...
      if (eventId.startsWith('temp-')) {
        toast({ 
          ...
          description: "Este é um evento temporário de teste.", 
          variant: "destructive" 
        });
        return;
      }
      ...
      try {
        ...
        if (response?.error) {
          throw new Error(response.error?.message || "Erro ao excluir evento");
        }
    ...
    [queryClient, refetchEvents, toast, selectedEvent],
  );
