...
const resolveDjName = (event) => {
  ...
  // Se tiver array de nomes de DJs, retornar todos
  if (Array.isArray(event?.dj_names) && event.dj_names.length > 0) {
    return event.dj_names.join(', ');
  }
  
  // Se tiver array de DJs, pegar os nomes
  if (Array.isArray(event?.djs) && event.djs.length > 0) {
    return event.djs.map(dj => dj.name || dj.artist_name || dj).filter(Boolean).join(', ');
  }
        ...
        <div className="space-y-4 px-6 pb-6 pt-6">
          ...
          {event?.creator_name && (
            <div className="flex items-center gap-3 text-sm text-white/70">
              ...
              <span>Criado por: {event.creator_name}</span>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-white/70">
            ...
            {/* Indicate if this fee generates a pending financial transaction or is informational only */}
            {event?.created_by_admin ? (
              <span className="ml-2 inline-flex items-center gap-2 rounded-full bg-amber-500/20 px-2 py-0.5 text-xs font-semibold text-amber-300">
                <span className="h-2 w-2 rounded-full bg-amber-400 block" />
                Pendência financeira
              </span>
            ) : (
              <span className="ml-2 inline-flex items-center gap-2 rounded-full bg-white/5 px-2 py-0.5 text-xs font-medium text-white/60">
                Valor informativo
              </span>
            )}
