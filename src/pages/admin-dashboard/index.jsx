...
import PaymentReviews from './components/PaymentReviews';
...
import { usePaymentStats } from '@/hooks/useFinancial';
...
const AdminDashboard = () => {
  ...
  // Payment stats (hook) - call at top-level
  const paymentStats = usePaymentStats(realtimePayments || payments);
  ...
  const eventStatusSummary = useMemo(() => {
    const eventsList = realtimeEvents || [];
  ...
  // Dados reais dos gr��ficos baseados nos dados do Supabase
  ...
  return (
    ...
    {/* Status dos Eventos e Resumo Financeiro */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {upcomingEventsSummary.length}{' '}
                    {upcomingEventsSummary.length === 1 ? 'evento' : 'eventos'} nos próximos 15 dias
                  </p>
                </div>
            ...
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              {(() => {
                ...
                const recebido = paymentStats?.paidRevenue ?? ((realtimePayments || []).filter(p => String(p?.status).toLowerCase() === 'paid').reduce((s, p) => s + (parseFloat(p?.amount) || 0), 0));
                const pendente = paymentStats?.pendingRevenue ?? ((realtimePayments || []).filter(p => String(p?.status).toLowerCase() !== 'paid').reduce((s, p) => s + (parseFloat(p?.amount) || 0), 0));
                const comissao = paymentStats?.totalCommission ?? (recebido * 0.20);
              ...
              <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                {(() => {
                  const total = (paymentStats?.totalRevenue) ?? (realtimePayments || []).reduce((s, p) => s + (parseFloat(p?.amount) || 0), 0);
                  const paid = (paymentStats?.paidRevenue) ?? (realtimePayments || []).filter(p => String(p?.status).toLowerCase() === 'paid').reduce((s, p) => s + (parseFloat(p?.amount) || 0), 0);
