  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem('company_settings', JSON.stringify(result.data));
      } catch (e) {
  ...
  const renderProfileTab = () => {
        ...
        try {
          ...
          if (user) {
            const { data } = await supabase
              ...
              .single();
            if (data) {
              setProfileData(prev => ({ ...prev, full_name: data.full_name || '', avatar_url: data.avatar_url || '' }));
            }
          }
        } catch (e) {
    ...
    const handleProfileUpdate = async () => {
      try {
        ...
        if (profileData._avatarFile) {
          const file = profileData._avatarFile;
          const fileExt = file.name.split('.').pop();
          const fileName = `admin_avatar_${user.id}_${Date.now()}.${fileExt}`;
        ...
        setProfileData(prev => ({ ...prev, avatar_url: avatarUrl, avatar_preview: '' }));
      } catch (error) {
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-full overflow-hidden border border-border bg-muted flex items-center justify-center">
                {(profileData.avatar_preview || profileData.avatar_url) ? (
                  <img src={profileData.avatar_preview || profileData.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="space-y-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!file) return;
                    const previewUrl = URL.createObjectURL(file);
                    setProfileData(prev => ({ ...prev, _avatarFile: file, avatar_preview: previewUrl }));
                  }}
                  className="text-sm text-muted-foreground"
                />
                <p className="text-xs text-muted-foreground">Use uma imagem quadrada para melhores resultados</p>
        ...
        {editing && (
          <Button onClick={handleProfileUpdate} iconName="Save" iconPosition="left">
            Salvar Perfil
          </Button>
        )}
      </div>
