...
serve(async (req) => {
  ...
  try {
    ...
    const { eventId, action, reason, receipt_url, payment_method } = body;
    ...
    if (action === 'accept') {
      const updates: any = { payment_status: 'pago', payment_verified_by: user.id, payment_verified_at: new Date().toISOString() };
      if (receipt_url) updates.payment_proof_url = receipt_url;
      ...
      // Also attempt to update related transactions (if present) to mark as paid
      try {
        const txUpdates: any = { status: 'pago', paid_at: new Date().toISOString() };
        if (receipt_url) txUpdates.payment_proof_url = receipt_url;
        if (payment_method) txUpdates.payment_method = payment_method;
        await supabaseAdmin.from('transactions').update(txUpdates).eq('event_id', eventId);
      } catch (txErr) {
        console.warn('[review-payment] updating transactions failed (non-fatal):', txErr);
      }
