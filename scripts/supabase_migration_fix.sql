-- Migração para corrigir políticas RLS e estrutura de dados dos DJs
...
-- Criar view para compatibility com código existente
CREATE OR REPLACE VIEW djs AS
SELECT 
  id,
  name as artist_name,
  email,
...
-- Permitir acesso à view djs
CREATE POLICY "Allow access to djs view" ON public.djs
FOR ALL TO authenticated
USING (true);

-- Comentários
COMMENT ON VIEW public.djs IS 'View para compatibility com código legado que espera tabela djs';
COMMENT ON POLICY "DJs can view own profile" ON public.profiles IS 'Permite DJs visualizarem seus próprios perfis';
COMMENT ON POLICY "DJs can create events" ON public.profiles IS 'Permite DJs criarem eventos próprios';